...
class ImageCrops(torch.utils.data.Dataset):
    def __init__(self, image: Image.Image, objects,args,transform):
        self._image = image.convert("RGB")
        self.args = args
        self.transform = transform
        bgcolor = self.args.options.get('bgcolor', "#999")
        if bgcolor == "original":
...
def color_classification(image, bboxes, classname,args,transform):
    ...
    dataloader = torch.utils.data.DataLoader(
        ImageCrops(image, bboxes,args,transform),
...
def evaluate(image, objects, metadata,args,transform):
    """
    ...
    * CHANGED: Color and position will only be evaluated on the most confidently predicted objects;
    ...
    for req in metadata.get('include', []):
        ...
        if len(found_objects) < req['count']:
            ...
            if 'color' in req:
                # Color check
                colors = color_classification(image, found_objects, classname,args,transform)
...
def evaluate_image(filepath, metadata,args,transform):
    ...
    # Evaluate
    is_correct, reason = evaluate(image, detected, metadata,args,transform)
    return {
        'filename': filepath,
        'tag': metadata['tag'],
...
def main(args,transform):
    ...
    for subfolder in tqdm.tqdm(os.listdir(args.imagedir)):
        ...
        # Evaluate each image
        for num_image in range(4):
            imagename = f"{num_image+begin_idx:05}.png"
            imagepath = os.path.join(folderpath, sample_path, imagename)
            if not os.path.isfile(imagepath) or not re.match(r"\d+\.png", imagename):
                print("not evaluate at current image {}".format(imagepath))
                continue
            result = evaluate_image(imagepath, metadata,args,transform)
